...
- **Watchdog**: 상시 녹화기 FFmpeg `-progress pipe:3`(fd 3) 진행 업데이트의 마지막 수신 시각 기반. `FFMPEG_TIMEOUT` 초과 시 SIGTERM → 5초 후 SIGKILL. 로그(stdout/stderr)는 liveness에서 배제 — 무출력(로그 침묵)은 hang이 아니며(정상 무출력 오탐 없음, #68), 실제 동결(SIGSTOP 등)은 진행도 멈추므로 탐지된다. 상시 녹화기 전용이며 finalize 병합용 FFmpeg에는 적용되지 않는다.
...
| `FFMPEG_TIMEOUT` | `60` (초) | watchdog 진행(progress fd 3) stall 임계치 |
...
- FFmpeg watchdog은 조용히 멈춘(동결된) 프로세스를 복구하는 핵심. liveness는 상시 녹화기 `-progress pipe:3`(fd 3) 진행 수신 시각 기반이며, 로그에서 `FFmpeg progress stalled` 메시지 확인. 정상 무출력 녹화는 오탐하지 않음(#68).
