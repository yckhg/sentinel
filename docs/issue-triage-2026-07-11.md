...
- **#45·#47·#50·#56 (A 인프라 하드닝 묶음)** — 확정 진행: compose `build:` 추가(신규 호스트 `up` 한 번에) / 로그 로테이션 `10m × 3` (전 서비스) / mosquitto `mem_limit 256m` / hw-gateway non-root 하드닝 + 원문 payload 로깅 길이제한.
- **#42 (Go 단위 테스트)** — 별도 독립 작업 아님. **①/② 구현에 흡수**(구현하는 서비스마다 단위 테스트 동반).
- **#54 (레거시 문서)** — 스펙 정본화 정리(포인터 축소) 대상. `docs/interfaces/` SSOT 이중지위 정리와 함께.
- **#68 (watchdog 오탐)** — **보류**: 실물 RTSP 접합(#28) 완료까지 실검증 불가.
