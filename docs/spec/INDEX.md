...
| [archive-retention-policy.md](archive-retention-policy.md) — 영구 아카이브(`archives-data`) 무한증가 방지(GH #114; 용량 상한 `ARCHIVE_MAX_BYTES` oldest-first 주정책 + 나이 상한 `ARCHIVE_RETENTION_DAYS` 보조; `manual_*` 축출 제외; 롤링청소와 대칭·주기 자기수복; 삭제=기존 아카이브삭제 부작용 재사용·댕글링 없음; recording 델타) | `services/recording/archive_retention_test.go` (in-process 순수함수 판정; 단언 A~H·F2) | (5×3 교차검증 R4 HIGH0 수렴 · 구현 검증중) |
