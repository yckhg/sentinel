...
| 의존 대상 | 방식 | 필수 여부 |
|-----------|------|-----------|
| MQTT 브로커 (mosquitto) | subscribe + publish, client ID `sentinel-hw-gateway`(고정), **persistent session (clean session=false)** — 설계자 승인(mosquitto 세션 지속성 설정 필요), keep-alive 60s, 자동 재연결(지수 백오프 최대 60s) | 필수 — 브로커가 죽어도 HTTP 서버는 기동·응답을 유지(hang 없음)하되, `/healthz`는 미연결/미구독을 degraded(503)로 표면화한다("헬스체크(healthz) 계약" 참조) |
...
| `EQUIPMENT_EVICT_TTL_SEC` | `86400` | 마지막 수신(heartbeat/alert/candidate 중 최신) 이후 이 시간 초과 시 장비를 스토어에서 제거 (양의 정수만 유효, 그 외는 기본값). `HEARTBEAT_TIMEOUT_SEC`보다 충분히 크게 두어 dead 장비가 일정 기간 조회 가능하게 함 |
...
- **healthy(`200`, 본문에 `"status":"ok"`):** MQTT 브로커에 **연결**되어 있고, 필수 구독 4개 토픽(`safety/+/alert`, `safety/+/heartbeat`, `safety/+/alert/resolved`, `safety/+/event/candidate`)이 **모두 현재 구독 성립** 상태일 때에만.
- **degraded(`503`, 본문에 `"status":"degraded"`):** 브로커 미연결(최초 연결 전 또는 단절 후 재연결 진행 중)이거나, 4개 필수 토픽 중 하나라도 구독이 성립하지 않은 동안. 즉 gateway가 경보를 수신할 수 없는 상태는 항상 unhealthy로 표면화된다.
- **hang 금지:** `/healthz`는 브로커 상태와 무관하게 **유한 시간 내(수 초 이내)에 즉시 응답**한다 — 브로커 단절 중에도 무기한 블로킹하지 않는다.
...
- **불변식(무한 증가 금지):** in-memory 장비 스토어의 항목 수는 무한히 증가하지 않으며 항상 `EQUIPMENT_MAX_DEVICES` 이하로 유지된다. 오작동/악의적 publisher가 매 메시지마다 새로운 `siteId:deviceId` 키를 실어 보내더라도 메모리가 무제한으로 늘지 않는다.
- **TTL eviction:** 어떤 장비가 마지막으로 수신된 시각(heartbeat/alert/candidate 중 최신 서버 수신 시각)으로부터 `EQUIPMENT_EVICT_TTL_SEC`를 초과하면 스토어에서 **제거**된다. 제거된 장비는 `GET /api/equipment/status` 응답에서 사라진다(dead로 남는 것이 아니라 항목 자체가 없어진다).
- **상한 eviction:** 새 키 등록으로 항목 수가 `EQUIPMENT_MAX_DEVICES`를 초과하려 하면, least-recently-seen(가장 오래 수신 없는) 장비부터 제거하여 상한을 유지한다.
- `EQUIPMENT_EVICT_TTL_SEC`는 `HEARTBEAT_TIMEOUT_SEC`보다 충분히 크게 설정하여, dead로 마킹된 장비가 제거 전까지 일정 기간 조회 가능하도록 하는 것을 의도한다.
...
- 재연결 성공 시 4개 토픽을 자동 재구독한다. 필수 토픽 구독이 성립하지 않은 동안은 `/healthz`가 degraded(503)로 이를 표면화한다.
...
- **A. 헬스체크 (정상):** 브로커 연결·4개 필수 토픽 구독이 성립한 상태에서 `curl -s -o /dev/null -w '%{http_code}' $GW/healthz` → `200`, 본문에 `"status":"ok"` 포함.

- **A2. 헬스체크 degraded (MQTT 미연결/미구독):** mosquitto 정지로 브로커에 연결되지 않은 상태에서 `curl --max-time 5 -s -w '%{http_code}' $GW/healthz` → **`503`**, 본문에 `"status":"degraded"` 포함하며 응답은 hang 없이 5초 이내에 반환된다. 이후 mosquitto 기동으로 연결·4개 토픽 재구독이 성립하면 별도 조치 없이 A(200/`"status":"ok"`)가 다시 성립한다.
...
- **O2. 브로커 단절 중 발행 타임아웃 503 (연결 성립 후):** 브로커 연결이 한 번 성립한 뒤 mosquitto를 정지한 상태에서 `curl --max-time 10 -X POST $GW/api/restart -d '{"siteId":"site1","deviceId":"T-O1"}'`는 **유한 시간(발행 타임아웃 5초 + 여유) 내에 `503`** `{"error":"MQTT publish timeout — broker unreachable"}`를 반환한다(무기한 hang 하지 않음 — curl exit 28 아님). 같은 상태에서 `GET /healthz`는 즉시 **`503`(`"status":"degraded"`)** 이다(hang 없음 — 브로커 미연결을 표면화). (설계자 승인 변경: 이전 계약의 무기한 발행 블로킹을 유한 타임아웃 503으로 대체)
...
- **R. TTL eviction:** `EQUIPMENT_EVICT_TTL_SEC`를 작은 값(예: 10)으로 설정한 인스턴스에서 장비 하나를 heartbeat로 등록한 뒤 TTL + eviction 주기 여유를 두고 재수신을 중단 → `GET /api/equipment/status`에서 해당 장비 항목이 **사라진다**(dead=false로 남는 것이 아니라 항목 제거). 같은 조건에서 TTL 이내에 재수신하면 항목은 유지된다.

- **S. 재연결 경계 수신측 유실 방지 (브로커 세션 지속성 의존):** gateway가 QoS1 이상으로 구독한 상태에서 gateway를 짧게 정지→재기동하고, 그 정지 구간 동안 하드웨어가 `safety/site1/alert`로 alert 1건을 발행한다. gateway 재접속 후(persistent session + 고정 clientID로 세션 큐가 재전송을 보장하면) 해당 alert가 `POST /api/incidents`로 forward되어 유실되지 않는다. 이 단언은 브로커(mosquitto)의 세션/큐 지속성 설정이 갖춰졌을 때에만 성립한다.
...
10. **healthz 의미 변경(경보 유실 은폐 제거) — 계약 강화.** `/healthz`를 "HTTP 서버 생존"에서 "MQTT 연결 + 4개 필수 토픽 구독 성립"으로 강화하여, 브로커 미연결/미구독을 degraded(503)로 표면화하도록 계약을 바꿨다(§헬스체크(healthz) 계약, 단언 A/A2/O/O2). compose healthcheck가 이 엔드포인트를 쓰므로, 컨테이너 healthcheck의 재시작/재기동 정책과의 상호작용(브로커 재기동 지연 동안 gateway 컨테이너가 unhealthy로 판정되어 불필요 재시작되지 않는지)은 배포 시 확인 권장.
