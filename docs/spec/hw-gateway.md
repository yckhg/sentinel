...
| web-backend `POST /api/incidents` | alert 수신 (notifier 호출과 병렬) | 시도당 타임아웃 10초, **transport 에러(연결 실패·타임아웃 등)에 한해** 최대 3회 재시도 (지수 백오프 1s 기점 ×2, ±25% jitter). HTTP 응답이 도착한 경우에는 상태코드가 4xx/5xx라도 **재시도하지 않는다** |
...
- **웹 → 현장:** `POST /api/alert/resolved` 수신 시 `siteId` 필수(누락 시 400). 보정(`resolvedAt`, `resolvedBy.kind="web"`) 후 `safety/{siteId}/alert/resolved`로 발행. 브로커 연결 상태별 응답(503/블로킹/500)은 아래 "MQTT 발행 API 공통 응답 계약"을 따른다.
...
- `siteId`와 `deviceId` 모두 필수 — 하나라도 없으면 400. 브로커 연결 상태별 응답(503/블로킹/500)은 아래 "MQTT 발행 API 공통 응답 계약"을 따른다.
- 성공 시 `{"status":"sent","topic":"safety/{siteId}/cmd/restart"}` 반환.

### MQTT 발행 API 공통 응답 계약 (restart · alert/resolved · test-alert)

브로커 연결 상태에 따라 세 발행 엔드포인트의 응답이 달라진다 (코드 실측):

- **최초 연결 미성립:** 프로세스 기동 후 브로커 연결이 **한 번도** 성립한 적 없는 동안은 `503` `{"error":"MQTT broker not connected"}`를 즉시 반환한다.
- **연결 성립 후 브로커 단절(자동 재연결 진행 중):** `503`이 아니다. 연결 검사를 통과하여 발행이 클라이언트 내부 큐(in-memory)에 적재되고, **재연결·전송 완료까지 HTTP 응답이 블로킹된다**(서버 측 응답 타임아웃 없음). 재연결이 성립하지 않는 한 요청은 반환되지 않는다.
...
- **O2. 브로커 단절 중 발행 블로킹 (연결 성립 후):** 브로커 연결이 한 번 성립한 뒤 mosquitto를 정지한 상태에서 `curl --max-time 5 -X POST $GW/api/restart -d '{"siteId":"site1","deviceId":"T-O1"}'`는 5초 내 응답하지 않는다(curl exit 28 — `503`이 아니라 발행 대기 블로킹). 같은 상태에서 `GET /healthz`는 즉시 `200`이다.

- **P. 휘발성:** hw-gateway 재시작 직후 `GET /api/equipment/status`는 빈 배열 `[]`이다.

## ⚠️ 리뷰 필요 (의도 불확실)

1. **notifier 재시도 부재 vs 문서 불일치.** 구현은 notifier forward를 재시도 없이 1회만 시도하고, web-backend forward는 transport 에러에 한해 3회 재시도한다. 그러나 레거시 서비스 가이드(`docs/services/hw-gateway.md` "Outbound Calls")는 notifier에 "exponential backoff + jitter 재시도"가 있다고 기술한다 — 어느 것이 의도인지 확정 필요. 본 스펙은 코드 동작(notifier 0회, web-backend 3회)을 계약으로 적었다. (web-backend 재시도 정책은 `docs/spec/interface-mqtt.md`의 장애 동작 기술과 정합.)
...
3. **`POST /api/test-alert`가 레거시 서비스 문서 HTTP API 표에 없음.** 현재는 `docs/spec/interface-web-api.md` 계약 15에 HTTP 계약이 있으나, 레거시 서비스 문서(HTTP API 표, Code Structure의 엔드포인트 목록)에는 없고, `docs/spec/interface-mqtt.md`는 테스트 발행을 ⚠️로만 다룬다(발행 방향·`alertId` 부재). 공개 계약인지, 비공개 내부용인지 확정 필요.

4. **dedup 등록 시점이 forward 성공 이전.** `alertId`는 forward 시도 전에 처리됨으로 기록된다. 따라서 notifier/web-backend forward가 전부 실패해도 동일 `alertId`의 MQTT 재전송(펌웨어 재전송 정책의 핵심 시나리오)은 24시간 동안 다시 forward되지 않는다. "재전송으로 유실 복구"라는 `docs/spec/interface-mqtt.md` 재전송 정책의 의도와 충돌할 수 있다.
...
8. **브로커 단절 중 발행 요청의 무기한 블로킹이 의도인지 불확실.** 연결이 한 번 성립한 뒤 브로커가 죽으면(자동 재연결 중) 발행 3종 API는 503을 반환하지 않고 재연결까지 HTTP 응답을 무기한 블로킹한다(서버 측 타임아웃 없음). 자연스러운 운영 시나리오(가동 중 브로커 다운)에서 503 대신 hang이 발생하는 셈이라, "미연결 503" 계약의 원 의도가 이 경우까지 포함하는 것이었다면 연결 상태 검사 또는 발행 타임아웃 보강이 필요하다. 본 스펙 본문과 단언 O/O2는 코드 실측(최초 연결 전에만 503, 이후 단절은 블로킹) 기준으로 적었다.
