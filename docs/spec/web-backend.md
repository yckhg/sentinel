...
- 생성(무인증 internal 경로): 요청에 `alertId`가 있으면 동일 `alertId`의 기존 사고를 반환하고 **중복 행을 만들지 않는다**(멱등, DB unique 인덱스로도 보강). 단, **현재 유일한 호출자인 hw-gateway는 `alertId`를 전송하지 않는다** — 이 DB 멱등 경로는 API 계약으로는 존재하나 실경로에서는 발화하지 않으며, 실운영의 중복 제거는 hw-gateway의 in-memory dedup이 담당한다. `deviceId`가 오면 devices 테이블에 upsert되며 soft-delete 상태였다면 자동 복구된다.
