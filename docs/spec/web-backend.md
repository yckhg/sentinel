...
- **동료 서비스 (HTTP, Docker 내부 네트워크)**: hw-gateway(재시작·해소 전파·테스트 알림), streaming(활성 스트림 목록), cctv-adapter/youtube-adapter(카메라 reload·상태), recording(녹화/아카이브/스토리지 proxy 대상, 아카이브 finalize), notifier(초대 이메일 발송).
...
| 입력 | 출처 | 성격 |
|------|------|------|
| REST 요청 (`/auth/*`, `/api/*`, `/internal/*`) | web-frontend, 내부 서비스 | 스키마 SSOT는 `docs/interfaces/web-api.md` |
| `POST /api/incidents` | hw-gateway | 위기 발생 보고 (무인증, 네트워크 격리 전제) |
| `POST /api/devices/seen` | hw-gateway | 장비 heartbeat/alert 관측 보고 (무인증) |
...
- **HTTP/WebSocket 인터페이스 전체** — 엔드포인트 경로·요청/응답 스키마·에러 코드 매핑의 소유자는 `docs/interfaces/web-api.md`이다. 본 스펙은 그 스키마를 재정의하지 않고, 그 위의 보장만 기술한다.
- **WebSocket push** — `crisis_alert`(사고 생성 시, 전 클라이언트), `incident_resolved`(해소 시, 전 클라이언트), `connected`(접속 직후 본인에게). 메시지는 `{type, payload, timestamp(RFC3339 UTC)}` 봉투를 가진다. push 전용이며 클라이언트가 보낸 메시지는 처리하지 않는다. 느린 클라이언트의 메시지는 유실될 수 있다(버퍼 64, non-blocking send).
- **SQLite 영속 상태** — 스키마의 SSOT는 코드 내 마이그레이션 목록이며, 버전 번호 기반으로 멱등 적용된다(적용 이력은 `_migrations` 테이블). 이미 적용된 버전은 재실행되지 않고, 각 마이그레이션은 트랜잭션 단위로 전부-또는-전무다.
...
- 생성(hw-gateway 발): `alertId`가 있으면 동일 `alertId`의 기존 사고를 반환하고 **중복 행을 만들지 않는다**(멱등, DB unique 인덱스로도 보강). `deviceId`가 오면 devices 테이블에 upsert되며 soft-delete 상태였다면 자동 복구된다.
...
- H. **alertId 멱등**: 같은 `alertId`로 2회 POST → 1회차 201, 2회차 200이며 동일 `id` 반환. `SELECT COUNT(*) FROM incidents WHERE alert_id='<a>'` → `1`.
...
- L. **재시작 사전 검증**: devices에 없는 `(siteId,deviceId)`로 `POST /api/equipment/restart` → 400 `device not registered`; soft-delete된 장비 → 400 `device has been deleted` (hw-gateway 호출 없음).
...
- Q. **토큰 생존성**: `JWT_SECRET` 미설정 상태에서 로그인 → 컨테이너 재시작 → 기존 토큰으로 `GET /api/healthz` → 여전히 200.
...
- S. **proxy 오류 계약**: recording 컨테이너 정지 상태에서 `GET /api/recordings/<key>` (인증 포함) → 502 `{"error":"failed to reach recording service"}`.

## ⚠️ 리뷰 필요 (의도 불확실)

1. **임시 링크 JWT가 일반 JWT 검증을 통과해 폐기(blacklist) 검사를 우회할 수 있음.** 두 토큰 종류가 동일 secret·동일 HS256으로 서명되고, 일반 JWT 파서의 claims 구조가 임시 토큰의 payload(`linkId`만 있고 `userId`/`role` 없음)도 관대하게 수용한다. 인증 미들웨어와 `/ws` 모두 일반 파서를 **먼저** 시도해 성공하면 temp 분기(blacklist 확인)에 도달하지 않으므로, 폐기된 임시 토큰이 `role=""`인 사용자로 `/api/*`·`/ws`를 계속 통과할 수 있어 보인다. 검증 제안: 임시 토큰 발급→폐기 후 `GET /api/incidents`에 그 토큰 사용 — 401이어야 하는데 200이 나오면 NOK.
