...
2. **사고(incident) 생명주기의 단일 심판** — 사고의 생성·해소(resolve) 상태 전이는 오직 이 서비스가 판정하고 기록하며, 웹 사용자와 현장 센서 어느 쪽이 해소했는지 귀속(attribution)을 남긴다.
...
| `POST /api/incidents` | hw-gateway | 위기 발생 보고 (`X-Internal-Token` fail-closed + 네트워크 격리 전제) |
| `POST /api/devices/seen` | hw-gateway | 장비 heartbeat/alert/candidate 관측 보고 (`X-Internal-Token` fail-closed) |
| `POST /api/incidents/{id}/resolve-from-sensor` | hw-gateway | 센서 버튼에 의한 해소 보고 (무인증, 네트워크 격리 전제) |
| WebSocket 접속 (`/ws?token=...`) | 클라이언트 | 일반 JWT 또는 임시 링크 JWT |
| 환경변수 | compose | `DB_PATH`, `JWT_SECRET`, `ADMIN_USERNAME`/`ADMIN_PASSWORD`, `HW_GATEWAY_URL`, `STREAMING_URL`, `CCTV_ADAPTER_URL`, `YOUTUBE_ADAPTER_URL`, `RECORDING_URL`, `NOTIFIER_URL`, `FRONTEND_URL` — 전부 미설정 시 Docker 서비스명 기반 기본값으로 동작. **`INTERNAL_TOKEN`** — `POST /api/devices/seen`·`POST /api/incidents`의 `X-Internal-Token` fail-closed 게이트가 검증하는 공유 시크릿(hw-gateway와 동일 값). 미설정 시 두 경로가 모든 요청을 `401`로 거부(fail-closed)하므로 두 서비스에 동일 값 주입 필수 |
...
- **WebSocket push** — `crisis_alert`(사고 생성 시, 전 클라이언트), `incident_resolved`(해소 시, 전 클라이언트), `connected`(접속 직후 본인에게), `device_reappeared`(삭제 장치 재신호 시 + 재접속 backfill, admin 전용). 메시지는 `{type, payload, timestamp(RFC3339 UTC)}` 봉투를 가진다. (WS 메시지 스키마의 SSOT는 `docs/spec/interface-web-api.md` 계약 14다.) push 전용이며 클라이언트가 보낸 메시지는 처리하지 않는다. 느린 클라이언트로의 push는 차단 없이 유실될 수 있으며, 한 클라이언트의 지연이 다른 클라이언트로의 전파를 막지 않는다.
...
- **설정 원자적 저장** — 여러 건강 임계값/설정을 한 번에 변경하는 벌크 저장은 **전부-또는-전무**다(단일 트랜잭션). 요청 내 하나라도 미지의 key거나 값 검증(타입·범위·교차제약)에 실패하면 어떤 key도 커밋되지 않아, 순차 개별 저장이 중간에 실패해 일부만 반영되는 부분 저장 상태가 발생하지 않는다. 벌크 검증 실패(미지 key 포함)는 리소스-부재가 아니라 요청 무효이므로 **`400`으로 통일**한다(단건 `PUT /api/settings/{key}`의 미지 key `404`와 구분). 각 key의 타입·범위·교차제약(무효값 판정)과 벌크 엔드포인트 스키마의 계약 소유자는 `docs/spec/interface-web-api.md`(계약 11 known-key 표)이다.

## 핵심 로직 (동작)

### 인증·인가
- 사용자 상태 기계: `pending → active`(admin 승인 또는 유효한 초대 토큰으로 가입 시 즉시) / `pending → rejected`. `active`가 아닌 계정은 올바른 자격증명으로도 로그인할 수 없다(403).
- 로그인 JWT는 24시간 유효, `userId`·`role`(`admin`/`user`)·`iat`(발급 시각, Unix epoch 초)를 담는다. `iat`는 자격증명 변경 경계 판정의 기준값이다(아래). `/api/*` 전체는 인증 미들웨어 뒤에 있으며, admin 전용 자원(연락처 CUD, 카메라 CUD, 현장, 초대, 설정, 링크 관리, 사고 ack/resolve, 삭제 장비 목록, **장비 관리(명시 등록 `POST /api/devices`·soft delete·alias PATCH)**, 테스트 알림)은 role까지 검사한다.
- **비밀번호 변경 시 기존 발급 토큰 무효화**: 사용자가 비밀번호를 변경하면, 그 사용자에게 **변경 시점 이전에 발급된 모든 로그인 JWT는 즉시 무효**가 되어 `/api/*`·직접 검증 경로(`/auth/pending|approve|reject|users`)·`/ws`에서 `401`로 거부된다. 이는 관측 가능한 **자격증명 변경 경계**로 성립한다 — 서버는 각 사용자의 경계를 **DB에 영속**한다(`users.password_changed_at`, credential_boundary). 인증 검증은 토큰의 `iat`가 소유 사용자의 `password_changed_at`보다 **이르면 거부**하고, 경계 이후 재로그인으로 발급된(=`iat`가 경계 이상인) 토큰만 유효하다. 경계가 **DB 영속**이므로 컨테이너 재시작 후에도 변경-이전 토큰은 부활하지 않고 계속 401이다(in-memory 경계였다면 재시작 시 탈취 토큰이 만료까지 부활). 변경에 사용한 토큰 자신도 이후 무효가 되어 클라이언트는 재로그인해야 한다. 탈취 토큰이 비밀번호 변경 후에도 만료(24h)까지 생존하지 못한다. (경계 데이터(`iat` 클레임 + `password_changed_at` 영속)와 비교 시맨틱이 계약이며, per-user token-version으로 등가 구현하는 것은 허용된다.) **비밀번호를 변경하지 않은 사용자의 토큰은 만료 전까지 계속 유효하다(단언 Q).**
...
### 사고 생명주기
- 상태 기계: `open → resolved`(중간 확인 상태 없음 — acknowledge 액션·상태는 계약에서 제거됨). **resolved는 종착 상태다** — resolved에 대한 재resolve는 409로 거부된다.
- 생성(무인증 internal 경로): 요청에 `alertId`가 있으면 동일 `alertId`의 기존 사고를 반환하고 **중복 행을 만들지 않는다**(멱등, DB unique 인덱스로도 보강). 이 멱등은 **동시성 하에서도 원자적으로 성립한다** — 같은 `alertId`로 N건이 동시에 도착해도 정확히 1건만 생성되고 나머지는 기존 사고를 에러 없이 `200`으로 반환하며, UNIQUE 부분 인덱스 충돌로 인한 5xx나 중복 행·유실이 발생하지 않는다(dedup 조회→삽입이 경쟁 창 없이 처리된다). 단, **현재 유일한 호출자인 hw-gateway는 `alertId`를 전송하지 않는다** — 이 DB 멱등 경로는 API 계약으로는 존재하나 실경로에서는 발화하지 않으며, 실운영의 중복 제거는 hw-gateway의 in-memory dedup이 담당한다. `deviceId`가 오면 devices 테이블에 **존재감** upsert되나 **sticky** — soft-delete 상태여도 복구되지 않고 `last_seen`만 갱신되며(`deleted_at` 불변), 삭제 장치의 최초 위기 재신호에는 `device_reappeared`가 1회 발행된다(seen 경로와 공유하는 재출현 가드).
- 웹 해소: admin만 가능, 해소 노트 선택(생략·빈 문자열 허용). 귀속은 `resolved_by_kind='web'` + 사용자 식별/표시명. 해소 성공 시 (a) recording에 아카이브 finalize 요청, (b) hw-gateway에 해소 사실 전파(센서 LED 등 동기화), (c) WS `incident_resolved` broadcast — 모두 비동기.
...
- **미해결 사고 조회(배너 backfill)**: 미해결(=`open`) 사고를 WS `crisis_alert` payload와 **동형**(식별자 `incidentId` + 중첩 `site:{address,managerName,managerPhone}`)으로 반환하는 전용 조회 접면을 제공한다 — web-frontend가 접속/재접속 시 실시간 push와 같은 모양으로 진행 중 위기 배너를 재구성(backfill)하며 반쪽 배너(현장 연락 정보 결손)가 되지 않게 한다. site 정보는 sites 테이블에서 조인한다. 스키마 계약 소유자는 `docs/spec/interface-web-api.md`(계약 2 `/api/incidents/active`)이다.

### 장비(devices) 영속
- 장비 등록 원천은 둘이다 — (a) **hw-gateway의 관측 보고로 자동 영속**(`POST /api/devices/seen`, `(site_id, device_id)` 유일), (b) **admin의 명시 등록/재활성**(`POST /api/devices`). 관리 변이(명시 등록·삭제·alias PATCH)는 **admin 전용**이다.
- **명시 등록/재활성**(`POST /api/devices`): 신규 `(siteId,deviceId)`면 `last_seen=NULL`(오프라인 대기)로 INSERT 후 `201`; 미삭제로 이미 존재하면 `409`; soft-deleted면 **재활성**(`deleted_at`·`reappear_alerted_at`을 NULL로, `last_seen`은 **불변** — 재활성이 존재감을 조작하지 않음, `alias`는 전송 시에만 갱신)하고 `200`. soft-deleted 장치를 관리로 되살리는 유일한 명시 경로다. `UNIQUE(site_id,device_id)` 위반은 원자적으로 `409`로 처리해 check-then-insert 경쟁 창을 닫는다.
- **sticky 삭제**: 삭제는 soft-delete(`deleted_at`)이며, 이후 관측 보고(`seen`·`incidents`)가 와도 **복원되지 않는다** — 존재감(`last_seen`)만 갱신되고 `deleted_at`은 non-null로 유지된다. 일반 목록은 미삭제만, `/all`(admin)은 삭제 포함 전체를 반환한다.
- **재출현 경보**: soft-deleted 장치가 재신호하면 WS `device_reappeared`(admin 전용)가 삭제→재출현 사이클당 **정확히 1회** 발행된다. 정확히-1회는 `reappear_alerted_at`의 rowcount-가드된 `NULL→now` UPDATE로 보장되며(clock 해상도·경로 도착 순서 무관), `seen`·`incidents` 두 경로가 **동일 가드를 공유**하므로 한 경로가 다른 경로의 경보를 잠식하지 않는다(정상 heartbeat 핫패스는 추가 쓰기 없음). 재활성 시 `reappear_alerted_at`이 NULL로 리셋되어 다음 사이클이 다시 1회 경보한다. 재접속한 admin에게는 현재 삭제+경보완료(`deleted_at IS NOT NULL AND reappear_alerted_at IS NOT NULL`) 장치의 `device_reappeared`를 **backfill 재전송**한다(전이 순간 미접속 admin의 유실 보정).
- **존재감(`last_seen`)과 생명주기(`deleted_at`)는 직교**한다 — 삭제 여부와 무관하게 보고는 `last_seen`을 갱신하고, 명시 등록된 미관측 장치는 `last_seen == NULL`(오프라인 대기). **안전 파이프라인 규율**: read 경로(목록·조회·health)는 **비게이팅**(삭제와 무관하게 계속 동작)이고, write 경로(`seen`·`incidents`)는 **sticky를 준수**하며 재출현을 발행한다. (스키마 SSOT는 코드 마이그레이션: `devices.last_seen`은 **nullable**, `reappear_alerted_at` 컬럼이 재출현 dedup 상태를 보유한다.)
...
- 미삭제 장비의 `last_seen` 나이가 임계(기본 60초)를 넘으면 해당 센서를 unhealthy로 판정한다. soft-delete된 장비는 감시 대상에서 즉시 제외된다. **판정(O3)은 `last_seen IS NOT NULL`인 센서에만 적용**한다 — `last_seen == NULL`(명시 등록됐으나 최초 heartbeat 미수신 = 오프라인 대기)은 unhealthy 후보가 아니라 판정에서 건너뛴다(존재감/생명주기 직교; `sensor-device-lifecycle.md` 단언 A2와 상호참조).
...
- **재접속 admin 스냅샷 재전달 (⚠ 권장 채택):** 위 전이 push는 전이 **순간 접속 중**인 admin에게만 닿아, 그때 미접속이던 admin은 통지를 영구 유실한다(위 불변식과 상충). 이를 보정하기 위해 admin WS가 새로 접속하면(`connected` 직후) 현재 `unhealthy`인 모든 감시 대상의 스냅샷을 그 admin에게 `system_alarm`(계약 14, `details.status=="unhealthy"`)으로 재전달한다. (⚠ 스냅샷은 in-memory이므로 web-backend 재시작 직후엔 재평가 전까지 비어 있을 수 있음 — 감수.)
...
- I. **resolved 종착성 + 해제노트 선택**: resolve된 사고에 재`PATCH .../resolve` → 409. acknowledge 엔드포인트는 계약에 없다(`PATCH .../acknowledge` → 404/405). `resolutionNotes`는 선택이므로 공백·생략으로 resolve해도 → 200.
...
- K. **sticky 삭제·재출현 경보**: soft-delete된 장비에 대해 `POST $B/api/devices/seen -d '{"siteId":"s1","deviceId":"d1"}'` → 200 후 `SELECT deleted_at FROM devices WHERE site_id='s1' AND device_id='d1'` → **여전히 non-NULL**(sticky — 복원되지 않음), 단 `last_seen`은 갱신됨. 삭제→재출현 사이클 최초 seen에서 WS `device_reappeared`가 정확히 1회 발행되고(`reappear_alerted_at` NULL→now), 같은 사이클 재-seen은 추가 발행하지 않는다. `POST $B/api/devices`(admin)로 재활성해야 `deleted_at`이 NULL로 복귀한다(복원의 유일 경로).
...
- O2. **unhealthy 전이 admin push**: admin WS 클라이언트 접속 상태에서 동료 서비스 1개를 내리고 임계 시간 경과 → 해당 admin WS가 `type=system_alarm` 메시지 1건 수신(payload `details`에 `entityKind=="service"`·해당 서비스 `entityId`·`status=="unhealthy"` 포함 — 계약 14 health-출처 하위 스키마). 같은 조건에서 user(비-admin) WS는 이 메시지를 수신하지 않는다. (자동 컨테이너 재시작은 ⚠️확인요망이므로 단언하지 않는다.)
- O3. **센서 unhealthy 표면화**: admin WS 접속 상태에서 등록된 센서의 heartbeat가 끊겨 `health.sensor_alive_threshold_sec`(기본 60s) 경과 → 그 admin WS가 `type=system_alarm`(`details.entityKind=="sensor"`·해당 `entityId=siteId:deviceId`·`details.status=="unhealthy"`) 1건 수신. user WS는 미수신.
- O4. **재접속 admin 스냅샷 재전달 (⚠ 권장)**: 어떤 감시 대상이 `unhealthy`인 상태에서 admin WS가 **새로 접속** → `connected` 직후 그 대상에 대한 `system_alarm`(`details.status=="unhealthy"`) 스냅샷을 수신한다(전이 순간 미접속이던 admin의 유실 보정). user WS는 스냅샷을 받지 않는다.
...
- Q2. **비밀번호 변경 시 토큰 무효화**: 사용자 U가 로그인해 토큰 `t`를 얻고 `t`로 `GET /api/incidents` → 200 확인. 이어 U가 `POST /api/auth/change-password`로 비밀번호를 변경 → 이후 **같은 `t`**로 `GET /api/incidents` → `401`; 변경 후 재로그인으로 얻은 새 토큰 → 200. **변경 후 컨테이너를 재시작해도 같은 변경-이전 토큰 `t`는 여전히 `401`**이다(경계 `users.password_changed_at` DB 영속 — 재시작으로 탈취 토큰이 부활하지 않음, 단언 Q와 대칭). 다른 사용자 V가 변경 전 발급받은 토큰은 영향받지 않아 여전히 200.
...
- V. **설정 벌크 원자성·검증**: `health.*` 임계 여러 건을 한 벌크 저장 요청으로 보내되 그 중 하나를 (a) 미지의 key, 또는 (b) 무효 값(비정수 `"abc"` / 하한 미만 `health.service_check_interval_sec="2"`(<5) / 교차제약 위반 `health.service_down_threshold_sec < health.service_check_interval_sec`)으로 만들어 저장 → **`400`**(리소스-부재 `404`가 아님 — 벌크 검증 실패는 요청 무효로 통일)이며 `SELECT key, value FROM system_settings WHERE key IN (...)`의 **어느 값도 변경 전과 달라지지 않는다**(부분 반영 0건). 같은 요청의 모든 key/값이 유효하면 → 200이며 전 항목이 반영된다. (interface-web-api.md 계약 11 known-key 타입/범위/교차제약 표와 교차.)
