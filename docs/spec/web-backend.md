...
- 로그인 JWT는 24시간 유효, `userId`·`role`(`admin`/`user`)·`iat`(발급 시각, Unix epoch 초)를 담는다. `iat`는 자격증명 변경 경계 판정의 기준값이다(아래). `/api/*` 전체는 인증 미들웨어 뒤에 있으며, admin 전용 자원(연락처 CUD, 카메라 CUD, 현장, 초대, 설정, 링크 관리, 사고 ack/resolve, 삭제 장비 목록, 테스트 알림)은 role까지 검사한다.
