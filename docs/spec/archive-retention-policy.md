...
- **아카이브 분류 (ID 접두어 기반 · `type` 필드 없음)** — `ArchiveMetadata`에는 `type` 필드가 없다. 분류는 **아카이브 ID(=`IncidentID` 접두어)** 로 한다:
  - **사고 아카이브(축출 대상)**: `incident_`로 시작하는 ID. 자동 사고 파이프라인만 이 네임스페이스를 발급한다(notifier가 `incident_{site}_{ts}`를 `incidentId`로 발급 → recording이 `{incidentId}_{streamKey}_{ts}`로 아카이브 ID 생성).
  - **비대상(보호)**: `manual_`로 시작하는 ID(수동 생성 시 `incidentId` 생략분에 서버가 부여), **및 `incident_`로 시작하지 않는 그 외 모든 ID**(사용자가 임의 `incidentId`를 명시한 수동 산출물 포함).
  - **네임스페이스 예약(불변식 · 생성 시점 강제)**: `incident_` 접두어는 **자동 파이프라인 전용으로 예약**된다. 수동 생성 경로(`POST /api/archives`)는 사용자 제공 `incidentId`가 `incident_`로 시작하면 **거부(400)** 한다(또는 서버가 비-`incident_` 네임스페이스로 재부여). 이로써 "비대상(사용자 산출물)은 결코 `incident_` 네임스페이스를 점유하지 않는다"가 생성 시점에 보장되어, 접두어 포함 규칙이 사용자 산출물을 자동 삭제하지 않는 안전한 분류가 된다. 이 예약이 없으면 축출이 사용자 명시 아카이브를 오삭제할 수 있다(비대상 보존 불변식의 전제).
...
- **주기 스케줄러** — recording은 이미 30초 티커 고루틴 2개를 돌린다: (1) `CleanupOldSegments`(`manager`), (2) `RefreshProtection`+`AutoFinalizeExpired`(`archiveManager`). 축출은 **별도 스케줄러를 만들지 않고 `archiveManager`를 이미 소유한 (2) 고루틴에 편승**하여 `AutoFinalizeExpired` **다음에** 1회 실행한다(같은 주기에 새로 `completed`된 아카이브도 그 주기 또는 다음 주기에 대상이 된다 — 자기수복이 순서 무관성을 보증).
...
| `ARCHIVE_RETENTION_DAYS` | 사고 아카이브 최대 보존 일수. 초과 아카이브는 용량 무관 축출. 정적기(신규 사고 없음)에는 대상이 0건까지 줄 수 있음(보존창 의미상 의도). | **없음(opt-in, 선택)** | `0` 또는 미설정 → 나이 축출 비활성 |
...
- **용량 불변식**: `ARCHIVE_MAX_BYTES > 0`이면, 대상 아카이브의 총 `SizeBytes` ≤ `ARCHIVE_MAX_BYTES`. **단, 용량 정책만으로 남는 대상이 1건뿐이면 그 1건의 크기까지 허용**한다(용량 정책의 oldest-first 삭제는 최신 완료 아카이브 1건을 항상 보존 — 단일 아카이브가 상한보다 커도 최신 증거를 지우지 않는다. 이 하한 케이스는 경고 로그로 표면화).
...
- **비대상 보존 불변식**: 축출 전 존재하던 모든 비-`incident_` 아카이브(`manual_*` 및 임의-id 수동 산출물)와 진행 중·`failed` 아카이브가 축출 후에도 그대로 존재한다.
...
- **주기 실행**: 축출은 외부 요청 없이 기존 `archiveManager` 30초 티커 고루틴에서 `AutoFinalizeExpired` 다음에 반복 실행된다(신규 스케줄러 없음). 초과 상태는 다음 주기 안에 자기수복(self-heal)된다.
- **결정적 진입점**: 축출 로직은 티커와 분리된 **직접 호출 가능 함수**로 노출한다 — 순수 선정 함수 `selectEvictions(archives []ArchiveMetadata, maxBytes int64, retentionDays int, now time.Time) []string`(삭제 대상 ID 반환, 부작용 0)과 그것을 `DeleteArchive`에 결선하는 얇은 래퍼 `EvictArchives(now time.Time)`. `now`는 주입 가능(나이/자기수복 판정 결정성). 이는 `CleanupOldSegments(window)`가 인자를 받아 티커 없이 1회 실행 가능한 것과 대칭이다.
- **`selectEvictions`는 전함수(total)**: `CreatedAt`가 빈 문자열이거나 RFC3339로 파싱 불가한 아카이브(구버전/손상 메타)는 **용량·나이 산정과 삭제 대상에서 제외**하고 경고 로그를 남긴다(파싱 불가를 근거로 증거를 오삭제하지 않는다 — 잔존은 운영자에게 로그로 표면화). 이로써 정렬·나이 판정이 결정적이다(신규 경로는 create·protect 양쪽에서 항상 `CreatedAt`를 채우므로 실제 대상은 파싱 가능).
- **용량 우선(주 정책)**: 대상(완료 `incident_*`) 총 `SizeBytes`가 `ARCHIVE_MAX_BYTES`를 초과하면, `(CreatedAt, ID)` 오름차순으로 가장 오래된 것부터 삭제해 총 용량을 상한 이하로 되돌린다(용량 루프의 하한: 최신 1건은 보존).
...
> 판정 전략: 삭제 대상 선정은 **순수 함수 `selectEvictions(...)`** 로 분리되어 부작용 없이 결정적이다. **A·B·C·D·E·G는 이 순수 함수의 in-process 테이블 단위 테스트로 판정**하고, **F(정합)와 삭제 배선은 `t.TempDir()`에 메타·디렉터리를 시드하고 `EvictArchives`→`DeleteArchive` 왕복으로 in-process 판정**하며, **H(주기 자기수복)도 티커 본문에서 추출한 `EvictArchives(now)`를 직접 호출하는 in-process 테스트로 판정**한다(형제 티커 작업 `CleanupOldSegments`·`AutoFinalizeExpired`가 in-process로만 판정되는 레포 선례와 동형). 티커가 `EvictArchives`를 배선한다는 사실은 시작 로그/순서 마커 + 코드 인스펙션으로 확인한다. 따라서 A~H 전건이 Docker·HTTP·mutating 격리 스택 없이 `golang:1.22-alpine` 컨테이너의 `go test`로 non-vacuous하게 판정된다(호스트 무설치). 시딩은 크기(`SizeBytes`)·`CreatedAt`·ID 접두어(`incident_`/`manual_`)·`Status`를 제어한 더미 메타로 구성한다.

- **A (핵심)** — 용량 상한: `ARCHIVE_MAX_BYTES = X`, 완료 `incident_*` 총 `SizeBytes`가 `X`를 초과하도록 시딩 → `selectEvictions`/`EvictArchives` 1회 → 이후 대상 총 용량 ≤ `X`(남은 대상 1건 하한 예외 포함, 그 경우 경고 로그). in-process: `SizeBytes` 합으로 관측.
- **B (핵심)** — 비대상 보존: `manual_*` 1건 + `incident_`로 시작하지 **않는** 임의-id 수동 1건 + 상한 초과 완료 `incident_*` 다수를 시딩 → 1주기 → 세 비대상(둘)이 여전히 존재하고 목록에 남아 있다. **추가로** 수동 생성 경로에 `incidentId="incident_..."`를 주면 생성이 **거부(400)** 되어 비대상이 애초에 `incident_` 네임스페이스를 점유하지 못함을 판정한다(네임스페이스 예약 불변식).
...
- **D** — 나이 상한: `ARCHIVE_RETENTION_DAYS = R`, 용량은 상한 이내로 두고 `CreatedAt` 나이가 `R`을 초과한 완료 `incident_*` 1건 + `R` 이내 1건을 시딩(주입 `now`) → 1주기 → 초과분 삭제, 이내분 보존. (경계 케이스: 나이 초과분이 유일·최신이라도 삭제됨 — 나이가 용량 하한을 무시.)
...
## 검증 스킵 선언 (정정 — 전건 판정 가능, SKIP 없음 목표)

- **A~H 전건 SKIP 아님**: 삭제 대상 선정이 순수 함수(`selectEvictions`)로 분리되고 부작용(F·삭제·주기)은 `t.TempDir()` + `EvictArchives`/`DeleteArchive` in-process 왕복으로 판정되므로, 8개 단언 모두 mutating 격리 스택 없이 **in-process 단위 테스트로 non-vacuous하게 판정**한다(레포 선례 `cleanup_test.go`·`archive_evidence_preservation_test.go`). `golang:1.22-alpine` 컨테이너에서 `go test` 실행(호스트 무설치).
- **격리 검증 스택은 선택(권장 아님)**: 형제 티커 작업이 in-process로만 판정되는 선례와 정합하도록, 별도 `verify/archive-retention/` docker 스택은 **필수가 아니다**. end-to-end REST 스모크(격리 compose + prod 무접촉 포트)로 A·B·F를 `GET /api/archives`/`GET /api/storage`로 이중 확인하고 싶으면 추가할 수 있으나, `GET /api/storage`는 manual 포함·15초 캐시라 대상-합의 직접 관측면이 아님에 주의(캐시 무효화/대상 실측 필요). in-process 판정이 이미 모든 단언을 닫으므로, 격리 스택으로도 판정 못 하고 **남는 스킵만** RESULT.md에 표면화한다.
