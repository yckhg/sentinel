...
- 핵심 의도는 두 가지다:
  1. **유실 방지** — 유효한 위기 이벤트가 접수되면, 각 연락처에 대해 반드시 "외부 채널 성공" 또는 "웹 시스템 알람 기록" 중 하나의 관측 가능한 결과가 남아야 한다. 조용히 사라지는 경로(silent fail)는 허용되지 않으며, 모든 실패는 로그에 남는다.
...
- 완전한 stateless — 영구 저장소 없음. 전송 이력의 유일한 1차 기록은 로그이며, 실패 시 웹 시스템 알람 경로로 web-backend DB에 감사 기록이 남는다.

## 의존 도구 · 시스템

| 시스템 | 용도 | 계약 소유자 |
|--------|------|-------------|
| web-backend (REST) | 비상연락처 목록 조회, 임시 CCTV 링크 발급, 시스템 알람 기록, site_url 설정 조회, 카메라 목록 조회 | `docs/interfaces/web-api.md` |
...
- hw-gateway가 MQTT `safety/{siteId}/alert` 페이로드를 그대로 전달한다. **페이로드 필드의 정의·의미는 `docs/interfaces/mqtt-publisher-guide.md`가 소유**하며 이 스펙은 재정의하지 않는다.
...
2. **연락처별 독립 fallback 체인** — web-backend에서 받아온 각 연락처마다 병렬로, 다음 순서를 보장한다:
   ...
   - SMS 성공 → 해당 연락처 완료.
   - SMS까지 실패·비활성·미설정 → **web-backend `POST /api/alarms`에 `notification_failure` 타입 시스템 알람을 반드시 기록**한다. 알람에는 연락처 식별 정보, siteId/deviceId, 두 채널의 실패 사유가 포함된다.
   - 즉, 외부 채널이 하나도 설정되지 않은 시스템에서도 위기 이벤트는 연락처 수만큼의 시스템 알람으로 반드시 관측된다.
...
8. **관측 가능성** — 접수, 채널별 성공/실패, degraded 진입, 시스템 알람 발송, 보호 요청 결과, dispatch 요약(성공 수/채널별 집계)이 모두 로그로 남는다. 어떤 실패도 로그 없이 소멸하지 않는다.
...
4. 연락처마다 병렬 실행:
   ...
   - KakaoTalk(활성 시) → SMS(활성 시) → 시스템 알람 순서의 순차 fallback. 성공한 첫 채널에서 종료.
5. 전 연락처 완료 대기 후 dispatch 요약 로그.
6. recording에 인시던트 세그먼트 보호 요청 (best-effort).

## 검증 단언 (TDD)

전제: sentinel 스택이 기동 중이고, 단언은 Docker 내부망의 다른 컨테이너에서 notifier(`http://sentinel-notifier:8080` 또는 compose 서비스명 `notifier:8080`)로 직접 주입한다. 관측은 `docker compose logs notifier` 및 web-backend의 alarms 조회로 한다.
...
- C. **최후 보루 보장**: `KAKAO_ENABLED`/`SMS_ENABLED`가 둘 다 true가 아닌 상태에서 연락처가 N(≥1)명일 때 유효 이벤트 1건 주입 → web-backend alarms에 `notification_failure` 알람이 정확히 N건 생성된다 (연락처당 1건, 각각 두 채널 실패 사유 포함). OK/NOK: 알람 건수 = 연락처 수.
...
3. **연락처 조회 경로가 인터페이스 문서와 불일치** — 구현은 무인증 `GET /internal/contacts`를 호출하지만, `docs/interfaces/web-api.md`에는 `GET /api/contacts`(인증 필요)만 기재되어 있고 `/internal/contacts`는 없다. 인터페이스 SSOT 갱신 또는 구현 정합 확인 필요.
4. **응답의 `contactCount`가 항상 0** — `/api/notify` 200 응답 body에 `contactCount: 0`이 고정으로 들어간다 (비동기라 시점상 알 수 없음). 의미 없는 필드를 계약에 남길지, 제거할지 결정 필요.
5. **엔드포인트별 접근 통제 비대칭** — `/api/send-email`은 내부 IP 검사로 보호되지만 `/api/notify`는 아무 통제가 없다. 네트워크 격리(내부망 전용) 전제라면 문제없으나, 한쪽만 방어하는 설계가 의도인지 확인 필요.
6. **녹화 보호 요청이 알림 완료 후 순차 실행** — 보호 요청은 전 연락처의 fallback 체인이 끝난 뒤에 나간다. 외부 API 타임아웃(채널당 최대 10초) 동안 보호 요청이 지연될 수 있다. 보호 대상 구간이 인시던트 이전 시간대이므로 실질 위험은 작아 보이나, 알림과 병렬로 즉시 보내는 것이 의도가 아닌지 확인 필요. 또한 이 동작 전체가 서비스 가이드에 상세 기재되어 있지 않다.
