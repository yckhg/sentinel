...
- 완전한 stateless — 영구 저장소 없음. 전송 이력의 유일한 1차 기록은 로그다. 실패 시 web-backend로 시스템 알람 기록을 추가로 시도하지만, 그 성공은 보장되지 않는다(§출력 2, ⚠️ 3 참조).
...
2. **연락처별 독립 fallback 체인** — web-backend에서 받아온 각 연락처마다 병렬로, 다음 순서를 보장한다:
   ...
   - SMS 성공 → 해당 연락처 완료.
   - SMS까지 실패·비활성·미설정 → web-backend `POST /api/alarms`로 `notification_failure` 타입 시스템 알람 기록을 **시도**한다. 시도 페이로드에는 연락처 식별 정보, siteId/deviceId, 두 채널의 실패 사유가 포함되며, 호출이 실패하거나 비 2xx 응답이면 그 결과가 로그로 남는다. **본 스펙이 보장하는 것은 "기록 시도 + 결과 로그"까지다** (수신측 현황은 ⚠️ 3 참조).
...
- C. **최후 보루 시도 + 실패 로그**: `KAKAO_ENABLED`/`SMS_ENABLED`가 둘 다 true가 아닌 상태에서 연락처가 N(≥1)명일 때 유효 이벤트 1건 주입 → notifier 로그에 시스템 알람 전송 시도의 결과 기록(성공 또는 `System alarm failed`)이 정확히 N건 남는다 (연락처당 1건). OK/NOK: 알람 시도 결과 로그 건수 = 연락처 수. (현재 스택에서는 수신 라우트 부재로 각 시도가 `status 401` 실패 로그로 종결되는 것이 정상 관측이다 — ⚠️ 3 참조. 알람의 DB 적재 여부는 단언하지 않는다.)
...
3. **의도된 "최후 보루"(웹 시스템 알람)의 수신측 부재** — 설계 의도는 두 외부 채널이 모두 실패한 연락처를 web-backend `POST /api/alarms`에 `notification_failure`로 영속 기록해 관리자가 시스템 알람으로 관측하게 하는 것이다. 그러나 web-backend에는 이 수신 라우트가 없고 `/api/*`는 인증 미들웨어 뒤에 있어, 무인증 호출은 **항상 401로 실패**한다 — 현재 최후 보루의 실체는 notifier 로그뿐이다 (`docs/spec/interface-web-api.md` ⚠️ 1과 동일 갭). web-backend에 internal 수신 라우트를 신설할지, notifier 측 호출을 제거할지 의도 확정 필요.
