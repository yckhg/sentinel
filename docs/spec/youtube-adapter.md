...
YouTube 영상 URL 또는 로컬 비디오 파일을 소스로 삼아, streaming 서비스의 RTMP 입력 규격에 맞는 영상 스트림을 지속적으로 push하는 것을 보장하는 demo/test 어댑터다. 실제 CCTV 없이도 시스템 전체(streaming → HLS → web)의 영상 파이프라인을 구동·시연할 수 있게 하는 것이 존재 이유다. 소스 특성상(YouTube 영상은 B-frame 포함) 반드시 재인코딩을 거쳐 규격을 맞춘다는 점이 계약의 핵심이다.
...
- **재인코딩 필수**: 어떤 소스든 출력은 항상 H.264(B-frame 없음) + AAC로 재인코딩된다. 소스 코덱을 그대로 통과(copy)시키지 않는다.
