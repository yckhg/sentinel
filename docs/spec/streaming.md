...
- 영구 저장소 없음. HLS 산출물은 컨테이너 내부 휘발 디렉토리에만 존재하며, **컨테이너 재시작 시 전부 초기화되는 것이 의도된 동작**이다. 녹화는 본 서비스의 책임이 아니다(별도 서비스가 RTMP를 직접 구독).
- 다른 서비스에 대한 런타임 의존이 없다 (DB 미접속, MQTT 미접속). 본 서비스는 홀로 기동·판정한다.

## 입력

- **RTMP push** — `rtmp://streaming:1935/live/{streamKey}`. 코덱/컨테이너/B-frame 금지 등 입력 규격의 소유자는 [docs/interfaces/streaming-api.md](../interfaces/streaming-api.md) "RTMP Input Specification"이다. 본 스펙은 규격을 재정의하지 않는다.
...
- **HLS 스트림** — `/live/{streamKey}/index.m3u8` + ts 세그먼트. URL 패턴·프래그먼트 길이·플레이리스트 길이·상대 URL 정책의 소유자는 [docs/interfaces/streaming-api.md](../interfaces/streaming-api.md) "HLS Output Specification"이다.
  - HLS 응답에는 `Cache-Control: no-cache` 및 `Access-Control-Allow-Origin: *` 헤더가 붙는다.
  - m3u8은 `application/vnd.apple.mpegurl`, ts는 `video/mp2t` MIME 타입으로 서빙된다.
- **`GET /api/streams`** — 항상 HTTP 200 + JSON **배열**을 반환한다. 스트림이 하나도 없으면 빈 배열 `[]`이다 (null 아님, 에러 아님). 요소 필드(`cameraId`, `streamKey`, `hlsUrl`, `active`, `startedAt`)의 의미와 active 판정 기준의 소유자는 [docs/interfaces/streaming-api.md](../interfaces/streaming-api.md) §4~5이다.
...
2. HTTP 8080이 유일한 외부 접점이다. `/live/`는 HLS 파일 서빙, `/healthz`와 `/api/*`는 내부 Go 프로세스(8081)로 프록시된다. **8080/8081 분리는 내부 구현이며 계약이 아니다** — 통합자는 8080만 알면 된다.
3. `/api/streams`는 HLS 출력 디렉토리를 스캔한다. 각 하위 디렉토리 = 스트림 후보이며, 그 안에 `index.m3u8`이 존재하는 것만 목록에 포함된다 (플레이리스트 없는 디렉토리는 조용히 제외).
4. **active 판정**: 플레이리스트 파일의 최종 수정 시각이 판정 창(기본 30초, 환경변수로 조정 가능) 이내면 `active: true`, 그 외 `active: false`. 이것이 alive/dead SSOT의 구현 정의다.
5. `startedAt`은 플레이리스트 최종 수정 시각을 UTC RFC3339로 반환한다. (→ 리뷰 필요 항목 2 참조)
6. HLS 디렉토리가 아예 없으면 (기동 직후 등) `/api/streams`는 에러 없이 빈 배열을 반환한다.
7. 컨테이너 기동 순서: HLS 디렉토리 생성 → Go API 백그라운드 기동 → nginx 포그라운드 기동. 재시작하면 이전 스트림 흔적은 남지 않는다.
...
- B. **무스트림 빈 배열**: 발행 중인 스트림이 없는 상태(예: 컨테이너 재시작 직후)에서 `curl -fsS http://streaming:8080/api/streams` 가 HTTP 200이고 본문이 정확히 `[]`(JSON 빈 배열)다.
...
- D. **active 판정 (SSOT)**: 단언 C의 push가 진행 중일 때 `/api/streams` 응답에 `streamKey == "spec-test"` 이고 `active == true` 인 항목이 존재한다. push를 중단하고 판정 창(기본 30초) + 여유 5초가 지난 뒤에는, 해당 항목이 목록에 없거나 있더라도 `active == false` 다.
...
- F. **HLS 응답 헤더**: 단언 C의 m3u8 응답(`curl -sI`)에 `Access-Control-Allow-Origin: *` 와 `Cache-Control: no-cache` 헤더가 포함되고, `Content-Type`이 `application/vnd.apple.mpegurl` 이다.
- G. **휘발성**: 스트림 발행 이력이 있는 상태에서 컨테이너를 재시작(`docker compose restart streaming`)하면, 직후 `/api/streams`가 `[]`를 반환한다 (이전 스트림 잔재 없음).
