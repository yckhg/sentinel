...
func TestCameraFanout_ThreeConsumers(t *testing.T) {
	...
	// the other two still receive + CRUD returns 2xx.
	t.Run("failure_tolerance", func(t *testing.T) {
		cctv := newReloadInstrument(t)
		yt := newReloadInstrument(t)
		closed := closedInstrumentURL(t)
		withConsumerURLs(t, cctv.srv.URL, yt.srv.URL, closed)

		r := adminReq(t, http.MethodPost, "/api/cameras", "", `{"name":"cam-fault","sourceType":"rtsp"}`)
		w := httptest.NewRecorder()
		handleCreateCamera(db)(w, r)
		if w.Code != http.StatusCreated {
			t.Fatalf("failure_tolerance: want 201 despite unreachable recording consumer, got %d (%s)",
	...
	t.Run("db_failure_no_dispatch", func(t *testing.T) {
		...
		// Give any (erroneous) dispatch a chance to land, then assert none did.
...
func TestIncidentsSchema_NoCameraRef_Static(t *testing.T) {
	...
	// No foreign key / cascade from any table to cameras.
	if regexp.MustCompile(`(?i)REFERENCES\s+cameras`).MatchString(s) {
		t.Errorf("migrations.go must not declare any FK REFERENCES cameras")
	}
	if regexp.MustCompile(`(?i)ON\s+DELETE\s+CASCADE`).MatchString(s) {
		// cameras is the only registry a camera-linked cascade could hang off;
		// the spec forbids any delete-cascade path touching evidence.
		t.Errorf("migrations.go must not declare ON DELETE CASCADE (camera delete must not cascade)")
	}
	// incidents must not carry a camera_id column anywhere.
