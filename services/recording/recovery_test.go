...
// newRecoveryTestManager builds an ArchiveManager backed by temp dirs, with a
// live RecordingManager for protection bookkeeping. Metadata is not loaded from
// disk; the caller sets am.archives directly.
func newRecoveryTestManager(t *testing.T, archives []ArchiveMetadata) (*ArchiveManager, *RecordingManager) {
	t.Helper()
	base := t.TempDir()
...
func TestRecoverArchives_ReprotectsValidSegments(t *testing.T) {
	...
	am, rm := newRecoveryTestManager(t, []ArchiveMetadata{arc})
...
func TestRecoveryOrdering_MarkersInOrder(t *testing.T) {
	am, _ := newRecoveryTestManager(t, nil)
...
func TestRecoverArchives_InvalidRange_ForcedFailed(t *testing.T) {
	...
	am, _ := newRecoveryTestManager(t, []ArchiveMetadata{arc})
